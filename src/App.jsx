...
// ── Config ────────────────────────────────────────────────────────────────────
// In live mode this hits the Python bridge server (bridge_server.py).
// You can also point directly at the ESP32 /sensordata endpoint if you skip
// the bridge: 'http://172.20.10.6/sensordata'
const BRIDGE_URL    = 'http://localhost:5000/sensordata';
const RESET_URL     = 'http://localhost:5000/reset';
const POLL_INTERVAL = 500; // ms
const DEMO_MODE     = false; // ← set false when bridge_server.py is running

// ── Demo path generation ──────────────────────────────────────────────────────
// Simulates a robot tracing the perimeter of a concave (C-shaped) object,
// then switching to convex and flat shapes in a cycle.
function buildDemoPath(tick, shape) {
  const pts = [];
  const steps = Math.min(tick * 2, 72);

  if (shape === 'CONCAVE SHAPE DETECTED') {
    // C-shape: full circle with a dent between 100°–180°
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => addLog('BRIDGE: Polling http://172.20.10.6/ every 500 ms', 'info'), 1400);

    const interval = setInterval(() => {
      demoTickRef.current += 1;
      const stage = DEMO_STAGES[demoStageRef.current];

      // Advance stage every ~stage.ticks ticks
...
function PathVisualization({ path, theta, isConcave, isComplete }) {
  ...
  // Scale path points to fit SVG viewBox
  let scaledPoints = [];
