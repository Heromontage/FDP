...
export default function SensorCard({
  ...
  isCenter    = false,
  delay       = 0,
  unit        = 'cm',      // ← NEW: display unit  (e.g. 'cm', '°', 'u')
  max         = 100,       // ← NEW: value range max for the progress bar
  sublabel    = null,      // ← NEW: small tag under the value
}) {
  ...
  // Proximity badge — only meaningful for distance readings (unit === 'cm')
  const distLabel      = unit === 'cm'
    ? (clamped < max * 0.2 ? 'CLOSE' : clamped < max * 0.6 ? 'MID' : 'FAR')
    : null;
  const distLabelColor = unit === 'cm'
    ? (clamped < max * 0.2 ? '#FFB4AB' : clamped < max * 0.6 ? '#FFBA20' : '#00D2FD')
    : colors.accent;
    ...
    >
      ...
      <div className="flex flex-col items-center gap-3.5">
        <div className="flex items-baseline">
          <span className="font-mono font-light tracking-tighter"
            ...
            {unit === '°'
              ? Math.round(clamped)          // heading: integer degrees
              : unit === 'cm'
              ? clamped.toFixed(1)           // distances: one decimal
              : Math.round(clamped)          // position: integer
            }
          </span>
        ...
        <div className="flex justify-between w-full font-mono uppercase"
          ...
          <span>0{unit}</span>
          <span>{sublabel ?? (isCenter ? 'PRECISION SCAN' : 'RANGE')}</span>
          <span>{max}{unit}</span>
        </div>
      </div>
      ...
      <div className="absolute right-0 top-0 bottom-0 overflow-hidden rounded-r-xl"
        ...
        <div className="absolute bottom-0 left-0 right-0 transition-all duration-500 ease-out"
          style={{ height: `${pct}%`, background: 'linear-gradient(to top, #00FF88, #00D2FD)', boxShadow: `0 0 6px ${colors.accent}55` }} />
      </div>
    </div>
  );
}